...
OUTPUT_PATH = (
    PROJECT_ROOT
    / "output"
    / "benchmark_selective_address_number_25k_refined.csv"
)
...
for _, row in source1.iterrows():
    ...
    query_name_tokens.append(
        get_tokens(
            row["norm_name"]
        )
    )

    query_address_tokens.append(
        get_tokens(
            row["norm_address"]
        )
    )
...
# Require shared number AND an additional
# supporting signal:
#
#   shared name token
#        OR
#   at least 3 shared address tokens
...
for s1_index in moderate_address_matches:
    ...
    shared_name = bool(
        name_tokens
        & query_name_tokens[s1_index]
    )

    shared_address_count = len(
        address_tokens
        & query_address_tokens[s1_index]
    )

    strong_address_overlap = (
        shared_address_count >= 3
    )

    if (
        shared_name
        or strong_address_overlap
    ):
...
print("\n" + "=" * 70)
print(
    "REFINED 25K ADDRESS + NUMBER BENCHMARK"
)
print("=" * 70)
...
print(
    f"Refined-rule-only mean   : "
    f"{mean_selective_only_candidates:,.2f}"
)

print(
    f"Final V2 + refined mean  : "
